...
(function($) {
	...
	function eventOverlap(start, end, events){
		...
		$.each(_events, function (booking_id, _items) {
		    ...
		    var _eventEnd   = _items[_items.length - 1][0] + _items[_items.length - 1][1];
		...
		if ($self.options._assistants_mode) {
			...
			$.each(data.events, function(k, e) {
				...
				$.each(e.items, function(j, item) {
					service_start = service_end;
					 ...
					 (parseInt(duration[0])*60*60 + parseInt(duration[1])*60)*1000;
                                        ...
                                        var addServiceToAssistants = function (service_start, service_end, classes, border_top) {
						...
						border_top:      border_top,
                                            ...
                                            if (obj._start > end.getTime() || obj._end < start.getTime()) {
                                                return;
                                            }
					    ...
					    count_items += 1;
                                            ...
                                            addServiceToAssistants(service_start, service_end, ' break-down no-border-top', true);

                                            service_start = service_end + breakDurationMs ;
                                            service_end   = service_start + Math.ceil(durationMs / 2);
                                            addServiceToAssistants(service_start, service_end, ' break-up no-border-top', false);

                                        } else {
                                            service_end = service_start + durationMs;
                                            addServiceToAssistants(service_start, service_end, ' no-border-top', true);
                                        }
				...
				_assistants.forEach(function (_events, _att_id) {
					...
					var _event_end   = _events[_events.length - 1][0] + _events[_events.length - 1][1];
					var checkifcontained = eventOverlap(_event_start, _event_end, assistants[_att_id]);
					if(checkifcontained){
					    assistants[_att_id].eventCount = Math.max(assistants[_att_id].eventCount,checkifcontained);
			...
			$.each(data.events, function(k, e) {
				...
				var count_items = e.items.length;
                                ...
                                $.each(e.items, function(j, item) {
                                    ...
                                    var addServiceToEvent = function (service_start, service_end, classes, border_top) {
                                            var objs = $self.applyAdvancedFilters([{
                                                id: 	  	 e.id,
                                                title:           k === 0 ? e.title : '',
                                                from:            k === 0 ? e.from : '',
					    ...
					    count_items += 1;
                                            ...
                                            service_end = service_start + Math.floor(durationMs / 2);
                                            addServiceToEvent(service_start, service_end, ' break-down no-border-top', k === 0);

                                            service_start = service_end + breakDurationMs ;
                                            service_end   = service_start + Math.ceil(durationMs / 2);
                                            addServiceToEvent(service_start, service_end, ' break-up no-border-top', false);

                                        } else {
                                            service_end = service_start + durationMs;
                                            addServiceToEvent(service_start, service_end, ' no-border-top', k === 0);
                                        }
                                });
...
*/
	...
	Calendar.prototype._update_day_prepare_sln_booking_editor = function() {
        ...
        $('.sln-icon--pen').off('click').on('click', function(e) {
		...
		$( ".day-event-item__calendar-day .sln-icon--plus-circle" ).on('click', function(event, triggered) {
			...
			if(md.css('display') == 'none') {
				var neededHeight = 230 + ($(event.target).siblings('.service_wrapper').outerHeight(true) || 0);
				...
				dayEvent.css('z-index', dayEvent.hasClass('day-event-main-block') ? 1000 : 999);
			}
			md.toggle();
			$(this).toggleClass('rotate');
		});

		$( ".booking_tool_item .sln-icon--trash" ).on('click', function(event) {
		    	event.preventDefault();
			...
			$(event.target).closest(".day-event").height(function(index, height) {
				return (height + 100);
			});
		});

		$( ".confirm-delete a.btn-ok" ).on('click', function(event) {
			event.preventDefault();
			var url = $(event.target).attr('href');
			$.get(url);

			$(event.target).closest('.event-item').trigger('mouseleave');

			calendar.view();
		});

		$( ".confirm-delete .close" ).on('click', function(event) {
			$(event.target).closest(".day-event").height(function(index, height) {
				return (height - 100);
			});
		...
		$(".booking_tool_item .sln-icon--user-check").on('click', function(event) {
			event.preventDefault();
			if ($(this).closest('.sln-free-version').length) {
			    return false;
			}
			...
			$.ajax({
				...
				success: function($data) {
					var iconCheckmark = $('.event-item[data-event-id="'+ bookingId +'"] .sln-icon--checkmark');
					if(!$data['on_process']) {
	...
	function showEventsList(event, that, slider, self) {
		...
		slider.slideUp('fast', function() {
			...
			console.log(self.getEventsBetween(parseInt(event_list.data('cal-start')), parseInt(event_list.data('cal-end'))))
