...
function initSalonCalendar(
    ...
    var DayCalendarHolydays = {
        ...
        createPopUp: function(status, firstEl, lastEl, els, rule) {
            $(".cal-day-hour-part").removeClass("to-select");
            $(".cal-day-click-tip").hide();
            ...
            button.css({
                top: top,
                right:
                    $("#cal-day-panel-hour").innerWidth() -
                    $(".cal-day-panel__wrapper").innerWidth() +
                    40,
                position: "absolute",
            });
    ...
    $("#sln-calendar-assistants-mode-switch")
        .on("change", function() {
            ...
            if (calendar.options._assistants_mode) {
                var assistants = {};
                calendar.options.events.forEach(function(event) {
                    event.items.forEach(function(item) {
                        if (item.attendant) {
                            assistants[item.attendant] = item.attendant;
                        }
                    });
                });
                calendar.options.on_page =
                    Object.keys(assistants).length * 2 + 1;
            } else {
