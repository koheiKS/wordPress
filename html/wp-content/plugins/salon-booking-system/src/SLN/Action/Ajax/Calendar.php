...
//                                SLN_Enum_BookingStatus::PENDING_PAYMENT,
    ...
    private function wrapBooking($booking)
    {
        ...
        try {
            ...
            $discount = $discountItems->getDiscountForBooking(strval($booking->getDate()), $booking->getBookingServices(), $booking->getCustomer());
        ...
        $ret = array(
            ...
            "amount"      => $format->moneyFormatted($booking->getAmount(), false, true),
            "discount"    => $format->moneyFormatted($discountAmount, false, true),
            "deposit"     => $format->moneyFormatted($booking->getDeposit(), false, true),
            "due"         => $format->moneyFormatted($booking->getRemaingAmountAfterPay() - $discountAmount, false, true),
